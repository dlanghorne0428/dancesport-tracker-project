...
class NdcaPremHeatlist(Heatlist):
    ...
    def load_heat(self, h, json_record="", comp_ref=""):
        ...
        number_string = json_record['Heat_Number']
        ...
        # find the heat time
        time_string = json_record['Rounds'][0]['Date_Time']
        time_fields = time_string.split()
        day_of_week = ""
        the_date = time_fields[0]
        the_time = time_fields[1] + time_fields[2]
        h.set_time(the_time, day_of_week, date_string=the_date)
    ...
    def get_heats_for_dancer(self, dancer, heat_data, comp_ref):
        ...
        for p in json_record['Result']['Partners']:
            partner_name_list = p['Partner_Name']
            if len(partner_name_list) == 0:
                ...
                print("Error parsing partner_name")
            ...
            shirt_number = p['Bib']
            #print(dancer.name + " " + partner_name + " " + shirt_number)
            ...
            for h in p['Entries']:
                if partner.name > dancer.name:
                    heat = Heat()
                    self.load_heat(heat, h, comp_ref)
